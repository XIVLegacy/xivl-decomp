...
def resolved_type(row: dict) -> str:
    """Prefer the RTTI correction when the enrichment sweep supplied one."""
    return row.get("rtti_type") or row["type"]
...
def main() -> int:
    ...
    if not rows:
        print(
            "error: recovered zero GAM parameters; refusing to overwrite output",
            file=sys.stderr,
        )
        return 1
    ...
    with out.open("w", newline="\n") as f:
        f.write("""// xivl-decomp - clean-room decompilation of FINAL FANTASY XIV 1.x client binaries
// Copyright (C) 2026  XIVLegacy Dev Team
//
...
""")
        ...
        kinds = sorted({cpp_for_type(resolved_type(r))[1] for r in rows})
        for k in kinds:
            f.write(f"    {k},\n")
        f.write("};\n\n")
