...
The repository gate rejects retail binaries and generated assembly from the
tracked tree.

## Ghidra extraction pipeline
...
keeps the working scripts in `tools/ghidra_scripts/` but gives them the same
...
writes `headless.log` plus `run.json`. The manifest records the binary name and
SHA-256, Ghidra version, JDK 21 version, script path and SHA-256, script
arguments and environment, analysis settings, timestamps, status, exit code,
and an `analysis_timed_out` boolean that remains independent of status. A lane
cites the run id and retains those values with any promoted observation. Raw
logs, projects, exports, and decompiled bodies remain gitignored local evidence
...
`config/ffxivgame.vtable_method_names.json` is generated from the tracked RTTI
and slot catalogs, and the ledger audits its 15,331 rows directly. It and
`config/ffxivgame.external_dependencies.json` are ignored deterministic
artifacts. CI rebuilds both from tracked inputs and checks that the tracked tree
has no resulting diff. The ignored outputs are not pinned in CI. Neither
generator reads another repository, a retail binary, or a Ghidra project.
...
Run the complete public gate on its own line:
