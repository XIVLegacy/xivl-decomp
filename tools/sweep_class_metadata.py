        ...
        cols.append(
            {
                ...
                "sig": sig,
                "offset_in_obj": int.from_bytes(
                    bin_data[col_off + 4 : col_off + 8], "little"
                ),
                "chd_ptr": int.from_bytes(
                    bin_data[col_off + 0x10 : col_off + 0x14], "little"
                ),
            }
        )
    return cols
...
def build_asm_function_index():
    ...
    function_name) pairs for mapping each RVA to its containing function."""
...
def classify_writes(writes, asm_index):
    ...
    for (fn_start, fn_name), offsets in by_fn.items():
        ...
        if len(offsets) == 1:
            ...
            ctors.append(
                {"fn_rva": fn_start, "fn_name": fn_name, "write_offsets": offsets}
            )
        elif len(offsets) == 2:
...
def main() -> int:
    sweep_path = os.path.join(REPO, "build", "dynamic_cast_callsites.json")
    if not os.path.isfile(BIN):
        print(f"error: missing retail executable: {BIN}", file=sys.stderr)
        return 1
    ...
    if slot_counts:
        slot_counts.sort()
        print(
            f"- Vtable slot count: min={slot_counts[0]} median={slot_counts[len(slot_counts) // 2]} max={slot_counts[-1]} (n={len(slot_counts)})"
        )
    ...
    by_complexity = sorted(
        results.items(),
        key=lambda x: -(x[1]["vtable_count"] + len(x[1]["ctor_candidates"])),
    )
    print("\n## Top-15 most-complex classes (by vtable count + ctor sites)\n")
    for name, info in by_complexity[:15]:
        vt_str = ", ".join(
            f"0x{v['rva']:x}({v['slot_count']} slots)" for v in info["vtables"]
        )
        ...
        print(
            f"- `{name}` - {info['vtable_count']} vtables [{vt_str}], "
