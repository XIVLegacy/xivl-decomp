...
The GAM extractor starts with generic `IntData.Value0` placeholders.
The dispatcher confirms that each Data class has real property names;
they are not present as standalone strings in `.rdata` but are loaded
indirectly from `.data` by the per-class dispatcher's jump table.
...
DISPATCHERS: dict[str, dict] = {
    ...
    #                   PASS Validated for CharaMakeData (26/26) and
    #                   Player (92/92).
    #
    #   "local_index"   prologue has no `ADD EAX, -<base>`; it just
    #                   `CMP EAX, <count-1>` then `JMP [EAX*4 + JT]`.
    #                   The input is a local 0..N index rather than the
    #                   global GAM id. Another slot owns the global-to-local
    #                   mapping (for example, PlayerPlayer slot 2). Because
    ...
    "PlayerPlayer": {
        ...
        # Covers ids 203 + 321..345 (via the JT) + 579 (via the JZ special
        # case). The larger slot 1 dispatcher at RVA 0x001da320 contains the
...
def main() -> int:
    ...
    # Load gam_params.json to look up the real GAM ids per namespace. The
    # dispatcher's case handlers are sorted by id, so the K-th .data PUSH
    ...
    with out_md.open("w") as f:
        ...
        f.write("Auto-generated by `tools/extract_paramnames_dispatch.py`.\n\n")
        f.write("Each Data class has a vtable slot in its `MetadataProvider`\n")
        f.write("that maps either a GAM id or a local 0..N index to a\n")
        f.write("property-name string in `.data`. We walk that dispatcher's\n")
        f.write("asm, extract the per-key string pointers from `PUSH <imm32>`\n")
        f.write("instructions, and dereference them.\n\n")
        f.write("## Id-keyed dispatchers (GAM id -> name)\n\n")
        for ns, rows in sorted(by_ns_id.items()):
            ...
            f.write("\n")
        f.write("## Local-index-keyed dispatchers (no GAM-id pairing yet)\n\n")
        f.write("These dispatchers take a 0..N local-index input rather\n")
        f.write("than a global GAM id. The local->global mapping lives in\n")
        f.write("a separate slot (typically slot 2 of the same\n")
        f.write("MetadataProvider); that mapping is not established. Names are\n")
        f.write("listed in dispatcher source order without GAM-id pairing.\n\n")
