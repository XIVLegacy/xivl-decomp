...
def _fails(
    ...
    try:
        return bool(
            verifier.verify(observation_path, expected_path, retail_path, protocol_path)
        )
...
def _source_row(protocol: dict) -> dict:
    return next(
        row
        for row in protocol["observations"]
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="retail-protocol-caller-test-") as raw:
        ...
        check(
            "workflow guards protected main",
            "python tools/verify_retail_protocol_caller.py --check-dispatch" in workflow
            and "if: github.event_name == 'workflow_dispatch'" not in workflow,
        )
        check(
            "workflow uses protected environment",
        ...
        check(
            ...
            and sum("/setup-retail-toolchain@" in action for action in shared_actions)
            == 1
            and sum(
        ...
        check(
            "local grant passes the token to the shared fetch action",
        ...
        check(
            "shared fetch action owns input identity",
            "RETAIL_INPUTS_REPOSITORY" not in workflow
            and "name: Verify private input identity" not in workflow,
        )
        check(
            "toolchain explicitly enables Ghidra", 'include-ghidra: "true"' in workflow
        )
        check(
            "analysis requires fetch and toolchain",
            "if: steps.fetch.outcome == 'success' && steps.toolchain.outcome == 'success'"
            in workflow,
        )
        check(
            "finalize precedes the local retained verifier",
        ...
        check(
            "artifact upload follows finalize and retained validation",
            "always() && !cancelled() && steps.finalize.outcome == 'success' && steps.retained.outcome == 'success'"
            in workflow,
        )
        check(
            "failed evidence result requires every evidence stage",
            "steps.fetch.outcome != 'success' || steps.toolchain.outcome != 'success' || steps.analysis.outcome != 'success' || steps.finalize.outcome != 'success' || steps.retained.outcome != 'success'"
            in workflow,
        )
        check(
            "upload keeps only name and path",
        ...
        check(
            "lane verifier remains local",
            "python tools/verify_retail_protocol_caller.py --input" in workflow
            and "python tools/verify_retail_protocol_caller.py --validate-retained-output _retail-staging"
            in workflow,
        )
        ...
        safe.mkdir()
        _write(
            safe / verifier.ATTESTATION_FILENAME,
            verifier.build_attestation("pass", "1" * 40),
        )
        check(
            "single sanitized retained file passes",
        ...
        sha = "1" * 40
        check(
            "main dispatch passes",
            not verifier.dispatch_errors(
        ...
        check(
            "feature ref fails",
            bool(
                verifier.dispatch_errors(
                    "workflow_dispatch", "refs/heads/feature", sha, sha
                )
            ),
        )
        check(
            "pull request event fails",
            bool(verifier.dispatch_errors("pull_request", "refs/heads/main", sha, sha)),
        )
        check(
