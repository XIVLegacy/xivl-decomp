...
def _decode_type_token(token: str) -> str:
    ...
    m = re.match(
        r"^U\?\$Array@U\?\$Blob@\$0([0-9A-Pa-p@]+)@GAM@Component@@\$0([0-9A-Pa-p@]+)@GAM@Component@@$",
        token,
    )
    if m:
...
def main() -> int:
    ...
    if not strings_path.exists():
        print(
            f"error: missing {strings_path}; run import_to_ghidra.py first",
            file=sys.stderr,
        )
        return 1
    ...
    for s in strs:
        ...
        if not matches:
            ...
            continue
        for m in matches:
            id_token, ns_chain, mid_section, decorator = (
                m.group(1),
                m.group(2),
                m.group(3),
                m.group(4),
    ...
    with out_csv.open("w", newline="") as f:
        wr = csv.DictWriter(
            f,
            fieldnames=["id", "ns", "paramname", "type", "decorator", "source_rva_hex"],
        )
        wr.writeheader()
        for r in rows:
    ...
    with out_md.open("w") as f:
        ...
        f.write(
            f"Auto-generated by `tools/extract_gam_params.py`. {len(rows)} unique\n"
        )
        f.write("`(id, namespace)` parameters recovered from `.rdata` mangled\n")
        f.write("type names. Each row is one wire-serializable game-state\n")
        f.write("property; the *semantic* interpretation lives in\n")
        f.write("the client systems that consume each property.\n\n")
        f.write("Type codes: `signed char` = i8, `short` = i16, `int` = i32,\n")
        f.write("`float` = f32, `double` = f64. Most properties carry a single\n")
        f.write("primitive value via `Component::GAM::DecoratorSimpleAssign<T>`;\n")
        f.write("more complex shapes use other decorators.\n\n")
