...
// (RVA 0x009a4160) + the `LobbyCryptEngine` 9 slots recorded in
...
// (possibly the surrounding RUDP2 / IpcChannel framework).
//
...
// `0x14, 0x00, conn_type_lo, conn_type_hi`, etc.).
...
namespace xivl_decomp::net::lobby_proto_channel
{

// =====================================================================
// ClientPacketBuilderHeader - 16 bytes, written by CPB::BuildHeader (slot 2).
// =====================================================================
//
...
// Client parsing identifies bytes 4..7 as packet_size and
// num_subpackets. Bytes 12..15 are typically zero on client traffic.
//
...
struct ClientPacketBuilderHeader
{
    // The binary unconditionally writes `0x14`, a constant marker with
    // unresolved flag semantics.
    std::uint8_t magic_or_flags; // = 0x14
    // The binary unconditionally writes `0x00`; no set value was observed.
    ...
    // specific (Lobby / Zone / Chat).
    std::uint16_t connection_type;
    // NOT written by BuildHeader. Caller-populated total packet size.
    std::uint16_t packet_size;
    // NOT written by BuildHeader. Caller-populated subpacket count.
    ...
    // literal `0x0A`. Bytes 12..15 are not written by this client and
    // are typically zero.
    std::uint32_t timestamp_lo;
    std::uint32_t timestamp_hi_unused; // typically 0
};

static_assert(sizeof(ClientPacketBuilderHeader) == 16,
              "ClientPacketBuilderHeader must be 16 bytes");

// =====================================================================
// ProtoChannelMessageHeader - 16 bytes recovered from the client.
// =====================================================================
//
// The CPB framework writes one ClientPacketBuilderHeader followed by one or
// more SubPackets. Each SubPacket starts with a 16-byte
// ProtoChannelMessageHeader followed by a body. Specific SubPacket types embed
...
struct ProtoChannelMessageHeader
{
...
static_assert(sizeof(ProtoChannelMessageHeader) == 16,
              "ProtoChannelMessageHeader must be 16 bytes");
...
// vtable[2] = void BuildHeader(ClientPacketBuilderHeader* out) - see semantics
//             documented above.
...
//             outbound path lives elsewhere (possibly the RUDP2 layer
...
struct ClientPacketBuilder; // opaque; concrete layout not established
...
// `BasePacket -> SubPacket -> GameMessage`, so it may be the inner
...
// =====================================================================
//
// The concrete-class slot map is reproduced by `config/ffxivgame.rtti.json`
// and `config/ffxivgame.vtable_slots.jsonl` (only one subclass exists in
// this build); zone and chat traffic is NOT encrypted (no concrete
// CryptEngine subclass for those channels in the binary).
//
// Slot layout:
...
//
// A 32-byte window contains four Blowfish blocks. The trailing 0..31 bytes
// of each encrypt or decrypt call are passed through without processing.
// Callers must not place meaningful encrypted content in that trailing range.
