...
2. **Channel dispatch through a stub vtable** - opcodes that
...
work-table side. This page covers the *dispatch* side: how packets
...
The dispatcher reads the opcode for routing, then calls a stub. The caller
reads the packet fields from the local DummyCallback's storage area; the stub
does not perform game-logic work.

## Why DummyCallback?

`DummyCallback` is the production callback, not a debug placeholder. Two
reasons explain this design:
...
The 49 Receiver classes contain no dedicated class for several common opcodes:
...
2. **The dispatch stub does not implement game logic.**
...
- `docs/net/actorimpl-receiver-dispatch.md` -
  documents the concrete "Lua engine wires them up" mapping - 35 of 42
  Receivers dispatch via specific vtable
  slots on `Component::Lua::GameEngine::{LuaActorImpl, NullActorImpl}`,
  including slot 56 = Kick, slot 57 = RunEventFunction, and slot 58 =
