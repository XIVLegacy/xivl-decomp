...
causing the kick to silently no-op. The byte is cleared by a subscriber swap,
not by a direct `MOV imm 0`.
...
- `docs/net/kick-dispatcher-clearer.md` - the related clearer analysis; the
  "subscriber pointer replacement" candidate is confirmed
