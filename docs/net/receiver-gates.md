...
The "-" entries indicate that the post-cast scan did not find a direct
`CALL rel32` within 64 bytes. These receivers may return after the cast, with
the cast triggering subclass-bound logic during the RTTI walk, or may reach a
handler through a conditional path outside the scan window.
...
For a silent-drop symptom, this map identifies the applicable gate pattern.
...
- **A1.1 (24 receivers)**: a payload targeting the wrong actor type can fail the cast to `MyPlayer`, then invoke the handler with `NULL this` and no effect. For example, an `AchievementPoint` packet sent to an NPC actor follows this path.

- **A1.0 (3 receivers)**: a failed cast takes the null-check path and returns without dispatch; it has no A1.1 null-handler risk.

- **A2.1 (5 receivers)**: the receiver handler runs unconditionally; any gate is in the downstream handler and may depend on receiver state set by an earlier packet.

- **A2.2 (3 receivers)**: the receiver navigates the engine root. A silent drop can reflect uninitialized downstream engine-root state.

- **A2.3 (1 receiver - ExecuteDebugCommandReceiver)**: probably authentication-gated. Client won't run debug commands unless authorised. This path is not relevant for normal gameplay.

## Application to SEQ_005

The SEQ_005 cinematic packet sequence has this classification:
