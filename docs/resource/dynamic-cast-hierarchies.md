# Dynamic-cast class hierarchies

This page reconstructs client class relationships from all 481
`__RTDynamicCast` (`FUN_009da6cc`) callsites in `ffxivgame.exe` by extracting
each `(SrcType, TargetType)` RTTI pair and resolving it through the PE data
section.

## TL;DR

**129 distinct RTTI Type Descriptor addresses** recovered in a
single sweep, revealing 6 major class hierarchies the engine uses for polymorphic
dispatch.
...
subclasses**, completing the Lua-bindable class registry. These rows account for
the RTTI entries in `docs/script/lua-class-registry.md`:
...
|   |
|   +-- StatusBase                           (RTTI 0x012c31f8) [sibling of ActorBase]
|   +-- CommandEventRelationControlBase      (RTTI 0x012bfd88) [additional]
|   +-- WidgetBase                           (RTTI 0x012c1964) [additional]
|   +-- ItemBase                             (RTTI 0x012c0864) [additional]
|   +-- SpreadSheet                          (RTTI 0x012c2308) [additional]
|   +-- Debug                                (RTTI 0x012c0324) [additional]
|   +-- Math                                 (RTTI 0x012c09e8) [additional]
|   +-- Global                               (RTTI 0x012c27c0) [additional]
|
...
- **`CutScene`, `DesktopWidget`, `Global`, `Math`, `SpreadSheet`,
  ...
  engine-utility Lua-bindables (per `lua-class-registry.md`).
- **3 engine-internal `LuaControl` variants** (`LuaTentativeControl`
  family) aren't in `lua-class-registry.md` - they're engine-only
...
- **11 have no known direct receiver** (CutScene, DesktopWidget,
  ...
  live elsewhere (candidate dedicated subsystems, not the receiver
  inventory)
...
+-- Sqwt::DependencyObject                   (RTTI 0x01269ef0)
    ...
    +-- Sqwt::UIElement                      (RTTI 0x01269eb0)
        +-- Sqwt::FrameworkElement           (RTTI 0x01269e8c)  [21 cast targets]
            ...
            |   +-- TextBox                  (RTTI address not established)
            |   +-- Primitives::
...
The engine-side actor hierarchy is confirmed by the sweep's cast edges:
...
`docs/net/work-field-inventory.md`. The runtime uses dynamic_cast
...
marker). These are candidate "data binders" for the debug HUD's
...
The `docs/script/lua-actor-impl.md` 90-slot interface has an implementation
pair. It also has a `NullActorImpl` (null-object pattern) used 6x when a script
needs an actor reference but none is bound.
...
These RTTI addresses complete the types described in
`docs/event/group-shared-work-system.md`.
...
- `docs/event/status-condition-receivers.md` -
  (`__RTDynamicCast` pair and cast-analysis technique)
- `docs/event/actor-substat-receiver.md` -
  (`LuaControl` as the System-namespace SrcType)
- `docs/script/lua-class-registry.md` - Lua class registry (every
  `*BaseClass` entry has a concrete C++ RTTI address)
- `docs/script/lua-actor-impl.md` - LuaActorImpl pair (RTTI addresses are
  confirmed)
- `docs/event/group-shared-work-system.md` - Group builders (RTTI addresses
  are recovered)
- `docs/net/work-field-inventory.md` - work-field kinds (
