...
all the open RTTI gaps in `docs/script/lua-class-registry.md`:
...
Confirms `docs/script/lua-actor-impl.md`'s 90-slot interface + impl pair.
**Surprise**: there's also a `NullActorImpl` (null-object pattern)
used 6x - when a script needs an actor reference but none is bound.
...
- `docs/script/lua-actor-impl.md` - LuaActorImpl pair (RTTI
  addresses now confirmed)
- `docs/event/group-shared-work-system.md` - Group builders (RTTI
