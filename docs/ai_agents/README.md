...
method used to recover it. Keep branch state, work queues, matching progress,
session narration, and private maintainer context out of the public tier.

The repository publishes findings, catalogs, headers that preserve evidence, and
analysis tools. It does not publish retail binaries, generated assembly,
Ghidra project state, or the local source reconstruction. The ignored `orig/`
and `asm/` paths are local-only inputs and outputs, not tracked placeholders.
...
The README [License](../../README.md#license) and
[Acknowledgements](../../README.md#acknowledgements) sections are the
...
| Repository gate and its limits | [verification.md](verification.md) |
