# Director and quest framework

This page explains how client director and quest Lua scripts run on C++
Lua-binding base-class scaffolding rather than a C++ director hierarchy.

## Key reframing - the directors / quests / NPCs are NOT C++ classes

OpeningDirector, ZoneDirector, and WeatherDirector are Lua classes rather than
C++ classes. RTTI search across the
...
etc. as bases bindable from Lua scripts. Each concrete director (e.g.
"OpeningDirector for man0g0 Limsa intro") is a Lua script that
subclasses one of these bases and is loaded at runtime.

The C++ side exposes the base classes (this binary) + the Lua VM glue that binds them
(also this binary).
...
`getMp`, etc.). The exact name-to-slot mapping requires tracing the
Lua method-registration table.
...
90 slots is roughly 3x the size of DirectorBase, suggesting the
...
The exact slot-name mapping remains unresolved and requires tracing the Lua
...
3. **The Lua method registration uses
   ...
   visible in the binary as its own RTTI entry. These entries identify
   every Lua method name.
...
- `docs/net/wire-protocol.md` - wire architecture (the GAM
