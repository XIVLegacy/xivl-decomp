...
Director.prog file prevents further disambiguation.
...
## Inferred-name boundary

The canonical Lua names remain identifiable through the methods called on
`self` in shipped `Director*BaseClass.prog` scripts and the metatable-build
initializer adjacent to registry builder `FUN_0078e3a0`.

## Cross-references

- `docs/event/director-quest-framework.md` - architecture
- `docs/script/lua-class-registry.md` - the Lua class registry that names DirectorBaseClass + the source path `/Director/DirectorBaseClass.prog`)
- `docs/net/sync-writer.md`
- `docs/script/lua-bytecode-format.md` - the `.prog` format - apply `unluac` to a shipped DirectorBase to disambiguate slot names)
- `docs/event/quest-dispatch.md` - sibling
