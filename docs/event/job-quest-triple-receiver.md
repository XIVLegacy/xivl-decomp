# JobQuestCompleteTriple receiver

This page maps the six-slot `JobQuestCompleteTripleReceiver` and its dispatch
pattern alongside the six-slot `UserDataReceiver`.
...
5-slot) documented in `docs/net/actorimpl-receiver-dispatch.md`:
...
Per `docs/net/actorimpl-receiver-dispatch.md`,
...
void LuaActorImpl::slot78(SomeArg *arg) {  // ECX=this, [EBP+8]=arg
    ...
    PUSH EDI
    CALL 0x00785bf0                           ; NOTE 2-step success-gated dispatcher
...
void FUN_00785bf0(LuaSomething *arg0, ReceiverInstance *receiver, BytePtr flag) {
    ...
    engine_root = navigate_engine_root(arg0);  // CALL 0x00cc7510, same trampoline as KickReceiver
...
| Byte | Where | Role |
|---|---|---|
| `[0x012c41af]` | KickReceiver identifies this as the **default kick result byte** | Gate 1 - "did the first-pass check succeed?" |
...
void slot5(this, arg0) {
    ...
    FUN_002f9520(this+8, arg0);           // NOTE real work
}
```
...
**3 x 12-byte entries**, each entry being one completed job quest
...
from `docs/net/actorimpl-receiver-dispatch.md`:
...
UserDataReceiver is also 6-slot but uses Pattern A, confirming that vtable
size alone does not determine dispatch shape.
...
- `docs/net/network-dispatch-paths.md` - the structural
