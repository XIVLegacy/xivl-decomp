# Event status-condition receivers

This page maps slot 1, `Receive`, of the two-slot `SetEventStatusReceiver` and
`SetNoticeEventConditionReceiver` classes in the SEQ_005 cinematic-body opcode
set.
...
- **Source type** (`G1 = 0x1270964`, RTTI Type Descriptor in `.data`):
  ...
  the **Lua-side actor base class**. Same namespace as
  `DirectorBase` (cf. `docs/event/director-base-hooks.md`).
...
KickReceiver). The gate is **implicit in the dynamic_cast**:
...
context's *derived type* matters and is an additional source of silent
...
- The unknown bytes at `[0x012c3f7a]` / `[0x012c3f7b]` / `[0x012c3f7c]`
  ...
  (semantic not established; candidate ASCII chars).
...
+-- (other *Base classes - not established)
```
...
3. **Step 8 - `ActorInstantiate` (ScriptBind)** is when the actor's
   Lua-side derived class is presumably instantiated (per the
...
It remains unknown whether the post-`ScriptBind` `DirectorBase` instance
...
receiver fan-out. The per-receiver target type G2 varies across the 45 globals
and supplies the "receiver -> target subclass" map in the receiver inventory.

## Cross-references

- `docs/event/kick-order-event-receiver.md` - KickReceiver decomp
...
- `docs/net/kick-receiver-offset-map.md` - KickReceiver
  instance layout (the `receiver[+0x80]` flag mystery)
- `docs/net/receiver-class-inventory.md` - inventory of all
  43 receivers
- `docs/net/seq005-receiver-gates.md` - SEQ_005-specific
  cross-reference (the audit that prioritized this decomp)
- `docs/event/director-base-hooks.md` - DirectorBase 34-slot
