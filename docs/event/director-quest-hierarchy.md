...
## Finding: per-instance scripts are nearly empty
...
The SEQ_005 path spans four layers:

1. Server-driven events (`KickEvent`, `EventStart`, `EndEvent`, and related events)
2. The `DirectorBaseClass` machinery handling those events
3. The accompanying quest script's `processEvent*` / `processTtr*` methods
