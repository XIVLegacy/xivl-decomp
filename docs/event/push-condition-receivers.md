# `SetPushEventCondition*Receiver` geometry
...
The three functions share a "pack stack args then call downstream handler"
shape but differ in:
...
Their distinct field offsets, argument counts, and FPU instruction sequences
make them separate implementations; PUSH-order register allocation also
...
- `docs/net/receiver-class-inventory.md` - the 43-receiver
  ...
  context (the three implementations have distinct layouts and call shapes)
