# EndClientOrderEvent receiver

This page maps `EndClientOrderEventReceiver`, the third receiver in the
cinematic dispatch chain with KickEvent and RunEventFunction, and establishes
the shared receiver pattern and its structural role.
...
This rules out "EndEvent silent drop" as the hang's root
...
## Sub-dispatcher findings

1. **`FUN_008a09e0`** (with a
...
2. **`FUN_008a10c0`.** Even
   ...
   **Updated end-event dispatch architecture summary:**
   ...
   FUN_008a13a0 (102-case event-type dispatcher)
     down key: event_type byte at staging[0]
     down each case loads receiver[+0x18+offset] (= MethodDescriptor*)

   FUN_008a09e0 (6-case INVOKE sub-dispatcher)
     down for stateless event types: tail-jump -> call thunk -> done
     down for stateful event types: tail-jump -> call thunk -> fall through

   FUN_008a10c0 (6-case CLEANUP sub-dispatcher)
     down for stateful event types (case 2 + 3): tail-jump to
       cleanup thunk at 0x004a0640 / 0x004a0660
     down for stateless event types: explicit RET (no-op)
   ```
...
- `docs/event/kick-order-event-receiver.md` - KickEvent receiver
  (slot-2 heavy) - the 5-slot pattern is documented here
- `docs/event/start-event-fn-receiver.md` -
  RunEventFunction receiver (slot-2 trampoline) - the inner
  handler contains the detailed cinematic dispatch logic
