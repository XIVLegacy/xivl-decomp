...
This page maps the `QuestBase` multiple-inheritance layout, constructor,
factory, and master Lua-class dispatcher.
...
`docs/script/lua-class-registry.md`. The registration function builds
...
| 28 | `FUN_006dcfd0` | `FUN_00a72a20` (no-op) | **Quest-specific Lua hook** |
| 29 | `FUN_006dbea0` | `FUN_006e1640` | Lua-callable hook |

The fact that QuestBase has a non-no-op at slot 28 while AreaBase
doesn't identifies slot 28 as a **Quest-only Lua method**. Its
semantic name remains unidentified.
...
its method N." The actual quest-data getter logic lives in another
class referenced by QuestBase. Identifying it requires tracing the
JMP target.
...
- `docs/net/sync-writer.md` - the typed Work-field
