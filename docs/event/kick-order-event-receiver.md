...
> NOTES **Addressing convention** (clarified after a
> Ghidra GUI mix-up): xivl-decomp's asm dumps and the
...
| 4 | `0x0049d260` | `0x0089d260` | 15 B | Predicate - compares `[ECX+8]` against `[0x0130c778]` (the `0xE0000000` `NO_ACTOR` sentinel - see section2 below), returns `SETNZ AL`. So slot 4 = "is this kick targeting a real (non-null) actor id?" - used by callers that want to skip processing of zero-target kicks. |
...
two-way. Receiver field `+0x80` is the critical "primary kick"
...
Particularly damning when the spawn packets were nullified by a subsequent
`DeleteAllActors`.
...
**Confirmed via Ghidra GUI.** Decompiles to:
...
suspicious actor-related false positives: it has a set/clear pattern in a
complex state-machine function but is not needed to identify the writer.
