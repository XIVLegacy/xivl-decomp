...
## Headline correction to the LPB declaration census

That doc said: > PlayerBase needs 94 + 76 (CharaBase inherited) about 170 distinct >
C++-bound method signatures.

**That framing was wrong.** The decomp here makes it clear:

They're two independent API layers in two independent VMs.
...
### Correction: client's LuaPlayer is NOT meant to mirror this API

The decomp here makes clear that's wrong - the two APIs serve different VMs:

- **Engine PlayerBaseClass** (94 C++ methods + 77 Lua wrappers) ->
  consumed by **client-side** shipped `.lpb` scripts via the
  binary's Lua VM

### What this decomp DOES reveal for client
...
- `docs/actor/scenario-monster-hierarchy.md` - companion (CharaBaseClass +
