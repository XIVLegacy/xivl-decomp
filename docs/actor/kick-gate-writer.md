...
The per-tick context initially made `FUN_00766f00` look inconsistent with a
spawn-time writer. The confirmed idempotent semantics below explain why the
function can run every tick.
...
### Earlier candidate ruled out (kept for reference)
...
void FUN_009018f0(this, arg) {   // ECX = this (= EDI), [ESP+0xc] = arg
    ...
    [EDI+0x5c] = 1;                     // NOTE SET KICK GATE only when queue empty
end:
    return;
}
```
...
- Semantically perfect for "kick gate": only sets +0x5c=1 when the
  container at this+0x8 is FULLY DRAINED. Matches the finding
...
**Caller `FUN_008f4ed0`** (527 B) is a large loop function and a candidate for
the per-frame actor-tick driver that calls FUN_009018f0 on each actor that has
...
operates on a sync-primitive class, not an actor. See "Earlier candidate
ruled out" above.
...
- `FUN_00549330` (5 write sites at +0x96d/+0xa0d/+0xa60): inconsistent
  with single-purpose actor-flag setter; candidate for Variant family
- `FUN_00559f90` / `FUN_00559fb0`: candidates for Variant family
...
- `FUN_00766f00`: per-tick
  context made it look unlikely, but remains a candidate for re-examination
  after the three additional candidate rulings.
...
The static byte pattern produces false positives because:
