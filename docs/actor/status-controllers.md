...
- Their names are identifiable via the delegate template argument, but
  their constructors are not directly recoverable with the standard
  vtable-write-pattern grep. The delegate constructors would need to be
  located and traced back to the embedded controllers.
...
The active pointer is not `array_1690[10]`. Those are
...
contains bone/attachment-point transforms, not status pointers.**
