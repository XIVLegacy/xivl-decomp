# Lua bytecode format in `.lpb` and `.prog`
...
- `unluac` (Java) - Lua 5.0/5.1/5.2/5.3 decompiler
- `luadec` (C) - same family
- `ChunkSpy` (Lua) - a Lua-bytecode pretty-printer

## File extensions in the wild

From the script tree layout in `docs/script/lua-class-registry.md`:
...
The two extensions coexist in the script tree paths. The
...
- The ~250 `Component::Lua::GameEngine::*` RTTI classes that
  ...
  `docs/event/director-quest-framework.md`.
...
path addressed by resource ID (per the resource ID system,
...
The `.prog` and `.lpb` files inside sqpack DAT files start with the `\x1bLua` signature
and are byte-identical to standalone files compiled by `luac51`.

## Cross-references

- `docs/event/director-quest-framework.md` - the C++
  Lua-binding base classes that consume these bytecode files)
- `docs/script/lua-class-registry.md` - the
